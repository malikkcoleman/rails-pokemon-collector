...
def create_csv_file():
    with open('BaseSetFisrtEdition.csv', 'w', newline='',
              encoding='utf-8') as f:
...
def append_to_csv_file(set_id, name, card_number, type, rarity, edition, price, image):
    ...
    """
    with open('BaseSetFisrtEdition.csv', 'a', newline='',
              encoding='utf-8') as f:
...
html_text = requests.get(
    'https://www.tcgcollector.com/cards/intl/base-set?cardSource=all&sortBy=cardNumber&releaseDateOrder=newToOld&cardsPerPage=30&displayAs=list').text

html_text_tcg = open('base_set_images.html', 'r')
...
for poke in pokemon:
    ...
    append_to_csv_file('1', name_text.strip(), set_number_text.strip(),type_text.strip(), rarity_text.strip(), edition, price_text.replace('$', '').strip(), image_urls[image_urls_index])
